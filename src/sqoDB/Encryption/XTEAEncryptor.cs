...
{
    internal class XTEAEncryptor : IEncryptor
    {
        ...
        {
            for (var i = off; i < off + len; i += 8) encryptBlock(bytes, bytes, i);
        }

        public void Decrypt(byte[] bytes, int off, int len)
        {
            for (var i = off; i < off + len; i += 8) decryptBlock(bytes, bytes, i);
        }
        ...
        {
            var y = (inb[off] << 24) | ((inb[off + 1] & 255) << 16) | ((inb[off + 2] & 255) << 8) |
                    (inb[off + 3] & 255);
            var z = (inb[off + 4] << 24) | ((inb[off + 5] & 255) << 16) | ((inb[off + 6] & 255) << 8) |
                    (inb[off + 7] & 255);
            ...
            outb[off] = (byte)(y >> 24);
            outb[off + 1] = (byte)(y >> 16);
            outb[off + 2] = (byte)(y >> 8);
            outb[off + 3] = (byte)y;
            outb[off + 4] = (byte)(z >> 24);
            outb[off + 5] = (byte)(z >> 16);
            outb[off + 6] = (byte)(z >> 8);
            outb[off + 7] = (byte)z;
        }
