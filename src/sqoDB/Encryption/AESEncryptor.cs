 ...
 class AESEncryptor  :IEncryptor
{
    ...
    private void encryptBlock(byte[] inBuff, byte[] outBuff, int off) {
        int[] k = encKey;
        int x0 = ((inBuff[off] << 24) | ((inBuff[off+1] & 255) << 16) | ((inBuff[off+2] & 255) << 8) | (inBuff[off+3] & 255)) ^ k[0];
        int x1 = ((inBuff[off+4] << 24) | ((inBuff[off+5] & 255) << 16) | ((inBuff[off+6] & 255) << 8) | (inBuff[off+7] & 255)) ^ k[1];
        int x2 = ((inBuff[off+8] << 24) | ((inBuff[off+9] & 255) << 16) | ((inBuff[off+10] & 255) << 8) | (inBuff[off+11] & 255)) ^ k[2];
        int x3 = ((inBuff[off+12] << 24) | ((inBuff[off+13] & 255) << 16) | ((inBuff[off+14] & 255) << 8) | (inBuff[off+15] & 255)) ^ k[3];
        ...
        outBuff[off] = (byte) (x0 >> 24); outBuff[off+1] = (byte) (x0 >> 16); outBuff[off+2] = (byte) (x0 >> 8); outBuff[off+3] = (byte) x0;
        outBuff[off+4] = (byte) (x1 >> 24); outBuff[off+5] = (byte) (x1 >> 16); outBuff[off+6] = (byte) (x1 >> 8); outBuff[off+7] = (byte) x1;
        outBuff[off+8] = (byte) (x2 >> 24); outBuff[off+9] = (byte) (x2 >> 16); outBuff[off+10] = (byte) (x2 >> 8); outBuff[off+11] = (byte) x2;
        outBuff[off+12] = (byte) (x3 >> 24); outBuff[off+13] = (byte) (x3 >> 16); outBuff[off+14] = (byte) (x3 >> 8); outBuff[off+15] = (byte) x3;
    }
