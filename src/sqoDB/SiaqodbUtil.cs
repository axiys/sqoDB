...
namespace sqoDB
{
    ...
    public class SiaqodbUtil
    {
        ...
        {
            var siaqodb = new Siaqodb(databasePath);
            ReIndex(siaqodb);
        }
#if ASYNC
        ...
        {
            var existingOids = siaqodb.GetUsedRawdataInfoOIDS();
            Expression<Func<RawdataInfo, bool>> predicate = ri => ri.IsFree == false;
            var existingOIDsOccupied = siaqodb.LoadOids<RawdataInfo>(predicate);

            //intersection
            existingOids.Sort();
            var oidsToBeFreed = new List<int>();
            foreach (var oid in existingOIDsOccupied)
            {
                var index = existingOids.BinarySearch(oid);
                if (index < 0) oidsToBeFreed.Add(oid);
            }
        ...
        {
            var existingOids = await siaqodb.GetUsedRawdataInfoOIDSAsync();
            Expression<Func<RawdataInfo, bool>> predicate = ri => ri.IsFree == false;
            var existingOIDsOccupied = await siaqodb.LoadOidsAsync<RawdataInfo>(predicate);
        ...
        private static void ShrinkNormal(Siaqodb siaqodb)
        {
            ...
            var file = siaqodb.GetRawFile();
            var memStream = new MemoryStream();
            var memStreamNew = new MemoryStream();
            var fullFile = new byte[file.Length];

            file.Read(0, fullFile);
            memStream.Write(fullFile, 0, fullFile.Length);

            for (var i = 0; i < allOrderByPos.Count; i++)
            {
                var rawi = allOrderByPos[i];
                var actualBytes = new byte[rawi.Length];
            ...
            for (var i = 0; i < allOrderByPos.Count; i++) siaqodb.StoreObject(allOrderByPos[i]);
        ...
        private static async Task ShrinkNormalAsync(Siaqodb siaqodb)
        {
            ...
            for (var i = 0; i < allOrderByPos.Count; i++) await siaqodb.StoreObjectAsync(allOrderByPos[i]);
            file.Length = memStreamNew.Length;
    ...
    public enum ShrinkType
    {
        ...
        /// <summary>
        ///     This includes Normal + ForceClaimSpace but also shrink of every db file; after this operation OID values of the
        ///     stored objects may change.
        /// </summary>
        Total
    }
