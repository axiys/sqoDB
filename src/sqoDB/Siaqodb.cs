...
savedObject(this, e);
     ...
     /// <param name="properties">properties to be stored</param>
        ...
        /// <param name="transaction">Transaction object</param>
        /// <returns>true if object was updated and false if object was not found in database</returns>
        public bool UpdateObjectBy(object obj, sqoDB.Transactions.ITransaction transaction, params string[] fieldNames)
        {
            lock (_locker)
            {
                if (fieldNames == null || fieldNames.Length == 0)
        ...
        /// <param name="transaction">Transaction object</param>
        /// <returns>true if object was updated and false if object was not found in database</returns>
        public async Task<bool> UpdateObjectByAsync(object obj, sqoDB.Transactions.ITransaction transaction, params string[] fieldNames)
        {
            bool locked = false; await _lockerAsync.LockAsync(obj.GetType(), out locked);
            try
            {
        ...
        internal sqoDB.Indexes.IBTree GetIndex(string field, Type type)
        {
            SqoTypeInfo ti = this.GetSqoTypeInfo(type);
        ...
        public IList<TIndex> LoadIndexValues<T, TIndex>(string fieldName)
        {
            ...
            sqoDB.Indexes.IBTree index = this.GetIndex(fieldNameAsInDB, typeof(T));
            if (index != null)
            {
                sqoDB.Indexes.IBTree<TIndex> indexT = (sqoDB.Indexes.IBTree<TIndex>)index;
        ...
        internal void ShrinkAllTypes()
        {
            lock (_locker)
            {
                ...
                foreach (SqoTypeInfo ti in existingTypes)
                {
                    if (ti.Type == typeof(sqoDB.MetaObjects.RawdataInfo) ||
                        ti.Type == typeof(sqoDB.Indexes.IndexInfo2) ||
        ...
        {
            Expression<Func<sqoDB.MetaObjects.RawdataInfo, bool>> predicate = ri => ri.IsFree == false;
            List<int> existingOIDsOccupied = this.LoadOids<sqoDB.MetaObjects.RawdataInfo>(predicate);
            SqoTypeInfo tiRawInfo = this.GetSqoTypeInfo<sqoDB.MetaObjects.RawdataInfo>();
            //dump object bytes
            Dictionary<int, byte[]> objectBytes = new Dictionary<int, byte[]>();
